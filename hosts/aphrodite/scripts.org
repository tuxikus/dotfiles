#+title: Scripts
#+author: tuxikus
#+property: header-args :mkdirp

* tumx-sessionizer

#+begin_src bash :shebang #!/usr/bin/env bash :tangle ~/.local/bin/tmux-sessionizer
...
is_worktree() {
  local path="$1"
  # Return 0 (success) if path is a Git worktree, else 1
  [[ -f "$path/.git" ]] && grep -q '^gitdir: ' "$path/.git"
}
...
elif [[ "$selected" =~ ^\[HOST\]\ (.+)$ ]]; then
  selected="ssh_${BASH_REMATCH[1]}"
fi

if is_worktree "$selected"; then
  display_name="$(basename "$(dirname "$selected")")/$(basename "$selected")"
  selected_name="${display_name//\//@}"
else
  display_name=$(basename "$selected" | tr . _)
  selected_name="$display_name"
fi
